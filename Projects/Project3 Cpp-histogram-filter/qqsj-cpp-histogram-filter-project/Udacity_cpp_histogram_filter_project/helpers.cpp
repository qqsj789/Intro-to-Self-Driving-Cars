...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	float cent_prob = 1.0 - blurring;
	float corn_prob = blurring / 12.0;
	float adjc_prob = blurring / 6.0;

	vector<vector<float>>window{{corn_prob, adjc_prob, corn_prob},
	                            {adjc_prob, cent_prob, adjc_prob},
								{corn_prob, adjc_prob, corn_prob}};
		...
		for (int j= 0; j< width; j++){
			...
			for (int dx: range){
				for (int dy: range){
					...
					float multpal = window[dx + 1][dy + 1];
					newGrid[new_i][new_j] += multpal * grid_val;
				}
			}
		}
	}
